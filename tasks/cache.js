...
var caches;
var APIGEE_ENTITY_SINGULAR = 'Cache';
var APIGEE_ENTITY_PLURAL = 'Caches';
var ENTITY_COMMAND_NAME = 'Caches'; //used to create exportCaches, import*, delete*
var APIGEE_ENTITY_REQUEST_PATH = '/caches';
      ...
      if (!error && response.statusCode == 200) {
        ...
        if (caches.length == 0) {
          ...
          for (var i = 0; i < caches.length; i++) {
            // Custom report
            var apigee_entity_url = url + "/" + encodeURIComponent(caches[i]);
            grunt.file.mkdir(filepath);

            //Call target server details
            grunt.verbose.writeln(APIGEE_ENTITY_SINGULAR + ' URL: ' + apigee_entity_url);
            request(apigee_entity_url, function(error, response, body) {
              if (!error && response.statusCode == 200) {
                grunt.verbose.writeln(APIGEE_ENTITY_SINGULAR + ": " + body);
                var apigee_entity_detail = JSON.parse(body);
                var apigee_entity_file = filepath + "/" + apigee_entity_detail.name;
                grunt.file.write(apigee_entity_file, body);
                grunt.verbose.writeln(APIGEE_ENTITY_SINGULAR + ' ' + apigee_entity_detail.name + ' written!');
              } else {
