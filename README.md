...
> The migration tool supports basic authentication, 2-factor authentication or bearer tokens when authenticating with your Apigee orgs. If you're using 2-factor authentication or bearer tokens, you will need to populate the 'token' property in your config.js with a valid token. Tokens can be obtained via the Apigee API directly or utilities such as 'get_token'. See [here](https://docs.apigee.com/api-platform/system-administration/auth-tools) for more information on generating tokens.
...
In the root directory of the repository you cloned, you'll find a config.js file. Edit that file to add values specific to your old and new orgs. Add the following kinds of information for each of your orgs -- the one you're exporting from and the one you're importing to. If you are are using the token property for bearer authentication, the password property can be left blank or omitted from config.js.
...
token | A valid token generated by a user with permission to log in to your org
